...
class FConv2D_3x3(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias):
        ...
        result = QNN.quantize(F.conv2d(torch.floor(input), QNN.quantize(weight), padding=1))
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[1]:
            grad_weight = F.conv2d(input.transpose(0, 1), grad_output.transpose(0, 1), padding=1).transpose(0, 1)
