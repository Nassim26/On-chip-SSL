...
def train_DIET_standard(net, device, config, embedding_dim=None):
    if embedding_dim == None:
        try: 
            embedding_dim = net.embedding_dim
        except: 
    ...
    W_probe = torch.nn.Linear(embedding_dim, config.num_classes).to(device)
    W_diet = torch.nn.Linear(embedding_dim, len(training_data), bias=False).to(device)
...
def train_simcirc(net, device, config, embedding_dim=None):
    ...
    plt.stem(np.linalg.eig(W_diet.weight.detach().cpu().numpy())[0])
    plt.show()
    ### End new....
